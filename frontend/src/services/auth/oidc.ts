...
const tokenEndpoint = () => `${env.oidc.authority}/connect/token`;

/** Build the /connect/authorize URL and stash the verifier + state for the callback. */
export async function beginLogin(): Promise<string> {
  ...
  return `${env.oidc.authority}/connect/authorize?${params.toString()}`;
}

/** Exchange the authorization code (validated against the saved state) for tokens. */
...
/** Rotate an access token using the refresh token. */
...
export function logoutUrl(): string {
  ...
  return `${env.oidc.authority}/connect/logout?${params.toString()}`;
}
